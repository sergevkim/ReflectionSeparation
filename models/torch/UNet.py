...
class UNet(nn.Module):
    ...
    def forward_and_compute_all(self, batch, device=None):
        output = self.forward(batch['synthetic'])

        loss_transmission = F.mse_loss(output['transmission'], batch['alpha_transmitted'])
        loss_reflection = F.mse_loss(output['reflection'], batch['reflected'])
        ...
        '''
        # todo: add VGG L2
        return loss, {'mse_transmission': loss_transmission.item(),
