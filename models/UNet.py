...
class UNet(nn.Module):
    ...
    def body(self, x):
        ...
        legacy3 = self.conv_down_5_1(legacy2)
        legacy3 = self.conv_down_5_2(legacy2)
        legacy3 = F.relu(legacy3)
        legacy3 = self.conv_down_6_1(legacy2)
        legacy3 = self.conv_down_6_2(legacy2)
        legacy3 = F.relu(legacy3)

#        legacy3 = self.conv_down_6(legacy3)
#        legacy3 = F.relu(legacy3)   # shape is 128 (actually we needn't legacy3)
        up = self.conv_up_1_1(legacy3)
        up = self.conv_up_1_2(legacy3)
        up = F.relu(up)
